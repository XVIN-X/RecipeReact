    ...
    const themes = [
        {
            ...
            "--text-color": "#0A0A0A",
            "--text-light": "#575757",
            "--background-img": "url(lightbg.jpg)"
        },
        {
                ...
                <div className="options-container">
                    { fontSizes.map((size, index) => (
                        <button key={index} className="btn" onClick={() => changeFontSize(index)}>
                            {size.title}
                            { fontSize === index && <span><i class="fa fa-check" /></span> }
                        </button>
                    ))}
                </div>
            </div>
                ...
                <div className="options-container">
                    { animationSpeeds.map((speed, index) => (
                        <button key={index} className="btn" onClick={() => changeAnimationSpeed(index)}>
                            {speed.title}
                            { animationSpeed === index && <span><i class="fa fa-check" /></span> }
                        </button>
                    ))}
                </div>
            </div>
